...
export class BtcWatcher extends EventEmitter {
    ...
    while (this.running) {
      ...
      if (this.checkpoint) {
        if (
          ...
          (this.checkpoint.btcInfo.height > 0 &&
            this.lastBLockDetected.btcInfo.height <
              this.checkpoint.btcInfo.height)
        ) {
          this.lastBLockDetected = this.checkpoint;
          this.logger.info(
            "Using CHEKPOINT: Starting to sincronize at BTC heignt",
            this.lastBLockDetected.btcInfo.height
          );
        } else {
          this.logger.info(
            "Discarting CHEKPOINT: Starting to sincronize at BTC heignt",
            this.lastBLockDetected.btcInfo.height
          );
        }
      }
      ...
      if (!this.lastBLockDetected) {
        this.logger.warn(
          "There is not block detected in DB, starting to detect from best block at height:",
          bestBtcBlock.btcInfo.height,
          "with hash:",
          bestBtcBlock.btcInfo.hash
        );
        await this.btcService.save(bestBtcBlock);
        this.lastBLockDetected = bestBtcBlock;
      }
      ...
      if (this.prevBlockProcessed) {
        ...
        this.logger.info(
          "Last BTC block detected is at height",
          this.lastBLockDetected.btcInfo.height
        );
        this.logger.info(
          "Best BTC block is at height",
          bestBtcBlock.btcInfo.height
        );

        if (
          this.lastBLockDetected.btcInfo.height < bestBtcBlock.btcInfo.height
        ) {
          let blockMissing = this.lastBLockDetected.btcInfo.height + 1;
          let blockAtHeightN: BtcBlock = await this.btcApi.getBlock(
            blockMissing
          );
  ...
  public async blockSuccessfullyProcessed(block: BtcBlock) {
    this.logger.info(
      "Saving BTC block as last detected in DB, at height",
      block.btcInfo.height
    );
