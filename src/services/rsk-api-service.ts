...
export class RskApiService {
  ...
  public async getBlocksByNumber(height: number): Promise<RskBlock[]> {
    var blocksInfo: any[] = await this.nod3.rsk.getBlocksByNumber(
    ...
    for (const blockInfo of blocksInfo) {
      ...
      blocks.push(
        new RskBlock(
          ...
          block.hash,
          block.parentHash,
          blockInfo.inMainChain,
          new ForkDetectionData(block.hashForMergedMining)
        )
      );
    }
  ...
  public async getRskBlockAtCertainHeight(
    forkBlock: RskBlock,
    rskBlockAtSameOrPrevHeight: RskBlock
  ): Promise<RangeForkInMainchain> {
    let bytesOverlaps = forkBlock.forkDetectionData.getNumberOfOverlapInCPV(
      rskBlockAtSameOrPrevHeight.forkDetectionData.CPV
    );
    ...
    let heightBackwards =
      Math.floor((rskBlockAtSameOrPrevHeight.height - 1) / 64) * 64 -
      jumpsBackwards;
