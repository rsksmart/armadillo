...
export class MainchainService extends BaseService {
  ...
  public async getLastItems(numberOfItems): Promise<BranchItem[]> {
    let robjectsToReturn: any[] = await this.store
      .getCollection()
      .find()
  ...
  public async save(items: BranchItem[]): Promise<boolean> {
    var response = true;
    try {
      await this.store.getCollection().insertMany(items);
    } catch (ex) {
      response = false;
    }
    return response;
  ...
  public async removeLastBlocks(n: number): Promise<string[]> {
    const removeIdsArray: any[] = await this.store
      .getCollection()
      .find()
      .limit(n)
      .sort({ "rskInfo.height": -1 })
    ...
    await this.store
      .getCollection()
      .deleteMany({ _id: { $in: removeIdsArray } });

    return removeIdsArray;
  }

  public async getAll(): Promise<BranchItem[]> {
    let items: any[] = await this.store
      .getCollection()
      .find({})
      .toArray();
