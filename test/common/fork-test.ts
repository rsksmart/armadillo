...
describe("Fork class test", () => {
  it("Test all methods", async () => {
    ...
    let forkItem5 = new ForkItem(btcInfo, new RskForkItemInfo(new ForkDetectionData(PREFIX + CPV + NU + "00000005"), 0));
    let forkItem6 = new ForkItem(btcInfo, new RskForkItemInfo(new ForkDetectionData(PREFIX + CPV + NU + "00000006"), 0));
    let forkItem7 = new ForkItem(btcInfo, new RskForkItemInfo(new ForkDetectionData(PREFIX + CPV + NU + "00000007"), 0));
    ...
    expect(fork.getLastDetectedHeight()).to.deep.equal(5);
    ...
    expect(fork.getLastDetectedHeight()).to.deep.equal(7);
